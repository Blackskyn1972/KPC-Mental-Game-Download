...
window.openScoutProfile=async function(id){
  kpcV28CurrentView={type:'profile',id};
...
window.openScoutEdit=async function(id=''){
  ...
  kpcV28CurrentView={type:'edit',id:id||null};
  return kpcV28BaseOpenEdit(id);
};
openScoutEdit=window.openScoutEdit;

function kpcV28RenderCurrent(){
  if(kpcV28Editing()) return;
  if(kpcV28CurrentView.type==='profile' && kpcV28CurrentView.id){
    kpcV28BaseOpenProfile(kpcV28CurrentView.id).then(()=>setTimeout(kpcV28InjectStatus,0));
  }else if(kpcV28CurrentView.type==='list'){
    kpcV28BaseOpenScouting().then(()=>setTimeout(kpcV28InjectStatus,0));
  }
}
...
function kpcV28ScheduleRefresh(){
  ...
  kpcV28DebounceTimer=setTimeout(async()=>{
    try{
      ...
      if(!kpcV28Editing()){
        kpcV28RenderCurrent();
        if(kpcV28CurrentView.type==='list'||kpcV28CurrentView.type==='profile') toast('Scouting KPC mis à jour ✓');
      }
    }catch(e){}
  },450);
}
